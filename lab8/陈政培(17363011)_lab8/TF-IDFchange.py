...
def classfy(vec2classfy,p0Vec,p1Vec,pClass1):
    p1 = np.sum(vec2classfy*p1Vec)+np.log(pClass1)
    p0 = np.sum(vec2classfy*p0Vec)+np.log(1-pClass1)
    if p1 > p0:
        return 1;
    else:
        return 0
...
def spamTest():
    ...
    for docIndex in trainSet:
        ...
        count_vector=tfvectorizer.fit_transform(docList[docIndex]) # Tf 矩阵
        ...
        trainMat.append(tfidf.transform(docList[docIndex]))
        trainClass.append(classList[docIndex])
    ...
    for docIndex in testSet:
        ...
        transformer = TfidfTransformer() # 转换Tf矩阵
        tfidf = transformer.fit_transform(count_vector) # 将TF转换成Tf-Idf
        testMat.append(tfidf.transform(docList[docIndex]))
