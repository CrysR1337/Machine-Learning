...
def KMeans(inputImg, clusterNum):
    ...
    with graph.as_default():
        ...
        storeClass = tf.compat.v1.assign(lastClass, imgClass)
        ...
        dist1 = tf.compat.v1.placeholder(tf.float64, shape=(imgNum, imgSize))
        dist2 = tf.compat.v1.placeholder(tf.float64, shape=(1, imgSize))
        ...
        for i in range (clusterNum):
            updateDistances.append(tf.compat.v1.assign(distances[i], calculateDist))
        ...
        getImgClass = tf.compat.v1.assign(imgClass, tf.cast(tf.argmin(distances, axis=0), tf.int32))

        # calculate new centroids
        points = tf.compat.v1.placeholder(tf.float64, shape=[None, imgSize])
        newCentroid = tf.reduce_mean(points, 0)
        # update centroids
        updateCentroids = []
        for i in range(clusterNum):    
            updateCentroids.append(tf.compat.v1.assign(centroids[i], newCentroid))
...
dataMat.columns=['x','y']
sns.relplot(x="x",y="y",data=dataMat)
plt.show()